...
clvd_all_text
grepl("name", clvd_all_text)
typeof(clvd_all_text)
...
This is another approach that I tried reading as binaries but I am honestly at a loss for how to handle this raw file.

```{r}

con <- file("../Data/Raw/cleveland.data", "rb")  # Open in binary mode
raw_lines <- character()  # Empty vector to store lines

repeat {
  line <- readLines(con, n = 1, warn = FALSE)
  if (length(line) == 0) break
  line <- gsub("\0", "", line, fixed = TRUE)  # Remove null bytes per line
  raw_lines <- c(raw_lines, line)
}
close(con)

# Merge into one string
clvd_all_text <- paste(raw_lines, collapse = " ")

# Split on 'name' token (as verified by you manually)
clvd_obs <- strsplit(clvd_all_text, " name ")[[1]]
```
