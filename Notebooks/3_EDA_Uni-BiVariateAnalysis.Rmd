...
for (var in num_vars) {
  ...
  test_result <- t.test(formula, data = df1)
  cat("=== T-test for", var, "===\n")
  print(test_result)
  cat("\n\n")
}
...
for (var in num_vars) {
  formula <- reformulate("has_disease", response = var)
  p_val <- tryCatch(t.test(formula, data = X_df)$p.value, error = function(e) NA)
  
  is_missing_flag <- paste0(var, "_missing") %in% colnames(X_df) | var %in% missing_flags
...
Based on recommendation, all variables recommended as 'Include' will be used kept. 
...
The final modeling set has 899 observations and 36 features including the target variable. 